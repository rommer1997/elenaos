/**
 * Generador de mensajes personalizados con Claude AI
 *
 * Usa Claude para generar mensajes de retención altamente personalizados
...
/**
 * Genera un mensaje personalizado de retención usando Claude
 */
...
/**
 * Construye el prompt para Claude con toda la información de la clienta
 */
...
/**
 * Parsea la respuesta de Claude y extrae el mensaje
 */
function parseClaudeResponse(responseText: string): GeneratedMessage {
  // Claude devuelve el mensaje directamente según nuestras instrucciones
  ...
  return {
    ...
    reasoning: 'Generado por Claude basado en perfil completo de la clienta'
  }
}
...
/**
 * Genera un mensaje de fallback si Claude no está disponible
 */
...
export async function generateClientInsights(
  ...
  try {
    ...
    return message.content[0].type === 'text'
      ? message.content[0].text
      : 'No se pudieron generar insights.'
  } catch (error) {
